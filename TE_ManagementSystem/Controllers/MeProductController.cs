        ...
        public ActionResult GetOldProductList()
        {
            ...
            if (!string.IsNullOrEmpty(searchValue))
            {
                oldproductsList = oldproductsList
                    .Where(x => x.NumberID.ToLower().Contains(searchValue.ToLower()) || x.ProdName.ToLower().Contains(searchValue.ToLower()) || x.KPN.ToLower().Contains(searchValue.ToLower())).ToList<ViewOldProduct>();
            }
